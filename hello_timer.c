...
static ssize_t show_time(struct kobject *, struct kobj_attribute *, char *);
static ssize_t store_time(struct kobject *, struct kobj_attribute *, const char *, size_t);

static void repeat(unsigned long);

static struct kobj_attribute times_attrb =
    __ATTR(interval, 0664, show_time, store_time);
...
static ssize_t show_time(struct kobject *kobj, struct kobj_attribute *attr,
                          char *buf)
{
    	return sprintf(buf, "%lu\n", times);
}

static ssize_t store_time(struct kobject *kobj, struct kobj_attribute *attr,
